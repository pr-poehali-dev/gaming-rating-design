            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1">
                  {["По полезности", "По дате", "По рейтингу"].map((s) => (
                    <button
                      key={s}
                      onClick={() => setReviewSort(s)}
                      className={`text-xs px-2.5 py-1 rounded-lg transition-all ${
                        reviewSort === s ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"
                      }`}
                    >
                      {s}
                    </button>
              ...
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {Object.entries(review.criteria).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="text-xs text-muted-foreground w-16">{criteriaLabels[key]}</span>
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground border-t border-border pt-3">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1 rounded-lg transition-all ${
                          helpfulVotes[review.id] === 'helpful' ? "bg-green-500/15 text-green-600" : "hover:bg-secondary"
                        }`}
                      >
                      ...
                      <button
                        onClick={() => handleHelpful(review.id, 'not')}
                        className={`flex items-center gap-1 px-2.5 py-1 rounded-lg transition-all ${
                          helpfulVotes[review.id] === 'not' ? "bg-red-500/15 text-red-500" : "hover:bg-secondary"
                        }`}
                      >
                        <Icon name="ThumbsDown" size={13} />
                        {review.notHelpful + (helpfulVotes[review.id] === 'not' ? 1 : 0)}
                      </button>
                    </div>
                  </div>
                ))}
                ...
                {reviews.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground bg-card border border-border rounded-2xl">
                    ...
                    <button onClick={() => setShowRateModal(true)} className="mt-3 text-sm text-primary font-medium hover:underline">
                      Написать отзыв
                    </button>
                  </div>
                )}
